...
- **End-to-end browser tests.** Considered for v1 but deferred. Playwright tests against `wrangler dev` + a test data repo, with OAuth bypassed by minting session cookies directly using the Worker's `SESSION_SECRET`. Would cover: signed-out renders sign-in card, signed-in renders pill bar + tasks, marking a task persists, dashboard renders for admin, "View →" renders read-only mode, sign-out clears session. The existing 30 unit tests cover the Worker logic well; v1 manual QA + production smoke test were judged sufficient for the pilot. Revisit if the system grows past 1-2 contributors or if regressions start slipping into production.
