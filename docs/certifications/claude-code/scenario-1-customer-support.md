...
**Root cause:** each tool (or third-party server) serializes its own outputs independently, with no shared format contract across them, so the inconsistency is structural rather than a one-off bug.
